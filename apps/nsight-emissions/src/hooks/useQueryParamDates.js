...
function useQueryParamDates() {
  ...
  useEffect(() => {
    ...
    if (!primaryEnd || !primaryStart) {
      dispatch(resetPrimaryEmissionsData());
    } else {
    ...
    if (!comparisonStart) {
      dispatch(resetComparisonEmissionsData());
    } else {
      // Check if comparison dates are valid
      const diffCount = primaryEnd.diff(primaryStart, 'months');

      // create a string to calculate dates with specific date interval string
      const dateIntervalCalculationString = 'month';
